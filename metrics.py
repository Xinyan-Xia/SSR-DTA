...
def get_metrics_reg(y_true, y_pred, title, with_rm2 = False, with_ci=False):
    metrics = dict()
    metrics[title + "_mse"] = float(mean_squared_error(y_true, y_pred))
    metrics[title +"_mae"] = float(mean_absolute_error(y_true, y_pred))
    metrics[title +"_medae"] = float(median_absolute_error(y_true, y_pred))
    metrics[title +"_mape"] = float(mean_absolute_percentage_error(y_true, y_pred))
    metrics[title +"_maxe"] = float(max_error(y_true, y_pred))
    metrics[title +"_r2"] = float(r2_score(y_true, y_pred))
    metrics[title +"_pearsonr"] = pearsonr(y_true.flatten(), y_pred.flatten())[0]

    if with_rm2:
        metrics[title +"_rm2"] = get_rm2(y_true.flatten().tolist(), y_pred.flatten().tolist())
    if with_ci:
        metrics[title +"_ci"] = get_cindex(y_true.flatten().tolist(), y_pred.flatten().tolist())

    return metrics


def get_metrics_cls(y_true, y_pred, title, transform=torch.sigmoid, threshold=0.5):
